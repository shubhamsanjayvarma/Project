            ...
            if (email.trim() === adminEmail.trim()) {
                try {
                    ...
                    if (createError.code === 'auth/email-already-in-use') {
                        // FIX: The user exists but typed the wrong password. Try the alternative spelling.
                        if (password === 'Second@Thirft9909') {
                            try {
                                const retry = await signInWithEmailAndPassword(auth, email.trim(), 'Second@Thrift9909');
                                return retry.user;
                            } catch (retryError) {
                                throw error;
                            }
                        }
                        throw error; // Original error (wrong password)
                    }
                    throw createError;
                }
            }
