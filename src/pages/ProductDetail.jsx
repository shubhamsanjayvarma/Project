...
const ProductDetail = () => {
    ...
    useSEO({
        ...
        description: product ? `${product.name} — ${product.description?.slice(0, 155) || 'Premium pre-loved fashion from Second Thrift'}` : '',
        path: `/product/${id}`,
    ...
    return (
        <div className="product-detail-page">
            ...
            {product && <JsonLd data={breadcrumbSchema([
                { name: 'Home', url: '/' },
                { name: 'Shop', url: '/shop' },
                { name: categoryName, url: `/shop?category=${product.category}` },
                { name: product.name }
            ])} />}
